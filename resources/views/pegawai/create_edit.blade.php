      ...
      <div class="row">        
        <div class="col-md-12">
          <div class="tile">
            <div class="tile-body">
            @if (isset($pegawai))
            <form class="form-horizontal" enctype="multipart/form-data" action="{{route('pegawai.update', $pegawai->id)}}" method="post">
            {{csrf_field()}}
            {{method_field('PUT')}}
            @else
            <form class="form-horizontal" enctype="multipart/form-data" action="{{route('pegawai.store')}}" method="post">
            {{csrf_field()}}
            @endif
            @php
                // dd($pegawai)
